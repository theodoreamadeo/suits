...
export async function getOutfitRecommendations(
  ...
  try {
    // Convert preferences to query string
    const params = new URLSearchParams({
      skin_tone_hex: preferences.skin_tone_hex,
      gender: preferences.gender,
      usage: preferences.usage.join(","),
      footwear_preference: preferences.footwear_preference,
    });
    const response = await fetch(
...
export const processBodyFrame = async (
  frameData: string
): Promise<BodyMeasurement | { message: string }> => {
  try {
    // Ensure frameData is a valid base64 string
    if (!frameData.startsWith("data:image")) {
      console.warn(
        "Frame data does not have the expected format. Adding prefix."
      );
      frameData = `data:image/jpeg;base64,${frameData}`;
    }

    const response = await fetch(
      `http://localhost:8000/api/body-measurement/process-body-frame`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    return {
      message:
        error instanceof Error ? error.message : "Failed to process body frame",
    };
  }
};

export const getLatestMeasurements = async (): Promise<
  BodyMeasurement | { message: string }
> => {
  try {
    const response = await fetch(
      `http://localhost:8000/api/body-measurement/latest`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    return {
      message:
        error instanceof Error ? error.message : "Failed to get measurements",
    };
  }
};

export const saveMeasurement = async (
  data: MeasurementSaveData
): Promise<{ success: boolean; message: string; filename: string }> => {
  try {
    const response = await fetch(
      `http://localhost:8000/api/body-measurement/save`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
export const convertMeasurementToCamelCase = (
